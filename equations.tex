...
\usepackage{helvet} 					% Skalierte Helvetica \sfdefault
...
\usepackage{graphicx}
\usepackage{color}     % Notwendig um .pstex_t-Dateien von xfig einbinden zu können!!!
%\usepackage[small]{caption2}
%\usepackage[tight]{subfigure}
\usepackage{units}
%\usepackage{esint}   % muss mach amsmath geladen werden
%\usepackage{isotope}
%\usepackage{ziffer} nur mit MikTex möglich
...
% nicht mehr nötig ab babel version 3.7
%\frenchspacing % kein Abstand nach Punkt
...
%\typearea[12mm]{current}     % Satzspiegel neu berechnen
\typearea[10mm]{current}

% Kein Einzug der ersten Zeile bei Beginn eines neuen Absatzes:
%\setlength{\parindent}{0pt}

% andere Bildunterschrift mit Hilfe von caption2
%\renewcommand{\figurename}{Abb.}
\addto\captionsngerman % siehe http://www.faqs.org/faqs/de-tex-faq/part8/
{
    \renewcommand{\figurename}{Abb.}%
...
%AbkÃ¼rzende Befehle im Mathemodus:

%Vektoren und Betragszeichen
...
\title{\Huge{Equations of fluid dynamics}}
\subject{Report}
\author{Andreas Maier}
\date{\today}
%\publishers{Advisor:\\ Prof. Dr. J. Niemeyer}
%\dedication{Im Andenken \\ an meine Großmutter \\ Gerda Maier}
\begin{document}
\maketitle
\tableofcontents
...
\label{eq:RTdiff}
\end{align}
\chapter{General compressible fluid}
...
\subsection{Global dissipation of kinetic energy}
...
\subsection{Local dissipation of kinetic energy}
...
\minisec{Global dissipation of kinetic energy $\mathcal{E}$}
\begin{align}
...
\minisec{Divergence equation (Equation of State)}
\begin{align}
\pdd{r_i}p= 
-\rho \frac{\partial^2}{\partial r_i \partial r_j}(v_i v_j)
+\frac{\partial^2}{\partial r_i \partial r_j}\sigma^*_{ij} 
...
\chapter{Fluid dynamics in one dimension}
\section{Balance equations in Eulerian form}
...
\label{eq:1detot}
\end{align}
\section{Balance equations in Lagrangian form}
...
\section{Levi-Civita-Symbol in 3D}
...
\chapter{Jacobian determinant}
\section{Definition}\label{jacobi}
...
\section{Time derivative of Jacobian determinant}\label{jacdt}
...
\chapter{Rate of strain tensor, rotation tensor}\label{rotstraintensor}
...
\label{eq:rsrcontr}
\end{align}
\chapter{Derivation of the stress tensor for a newtonian fluid}
...
\chapter{Stress tensor in cartesian coordinates for 1d, 2d and 3d}
\label{stress1d2d3d}
For a so called newtonian fluid it can be shown, that the stress tensor
...
\chapter{Stress tensor in comoving coordinates}\label{costress}
...
\chapter{A contradiction when computing the dissipation}\label{contradiss}
...
\chapter{Longitudinal and transversal projection of vectors}
...
\chapter{Some speculations about ...}
\section{Entropy}\label{entro}
...
\section{The divergence equation}\label{diveq}
...
\subsection{Incompressible newtonian fluid}
